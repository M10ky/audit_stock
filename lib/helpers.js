...
// ── Formatage compact des grands montants (anti-overflow KPI) ─────
// « 119 116 539 » → « 119,1 M » — conserve la valeur exacte via fmt().
// Seuils : ≥ 1e6 → M (1 décimale), ≥ 1e3 → k (1 décimale), sinon brut.
export function fmtCompact(n) {
  ...
  if (Math.abs(v) >= 1e3) {
    return `${(v / 1e3).toLocaleString('fr-FR', { maximumFractionDigits: 1 })} k`
  }
  return v.toLocaleString('fr-FR')
}

// Montant compact + unité : « 8,4 M MGA »
export function fmtMoney(n) {
  if (n == null || n === '' || isNaN(Number(n))) return '—'
  return `${fmtCompact(Number(n))} MGA`
}

// Montant exact (valeur brute en fr-FR) + unité : « 8 377 122,6 MGA »
export function fmtMoneyExact(n) {
  if (n == null || n === '' || isNaN(Number(n))) return '—'
  return `${fmt(Number(n))} MGA`
}

// ── Filtres ───────────────────────────────────────────────────

// ← Mirrors js/utils.js isActif() (Étape B) : null/undefined traité comme actif
// (rétrocompatibilité) — un produit est inactif uniquement si actif === false.
export function isActif(p) {
...
// Valeur courante d'une variable CSS (dépend du thème actif sur <html>).
