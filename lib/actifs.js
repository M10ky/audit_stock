...
import { getActifNumero, STATUS_PRET } from '@/store/pretsStore'
...
// FIX (parité vanilla — js/constants.js TRANSITIONS_ACTIF) : HORS_SERVICE →
// SORTI manquait. Sans cette transition, isValidTransition() refuse à tort le
// passage direct d'un actif Hors service vers Sorti (ex: matériel HS finalement
// cédé/jeté sans repasser par une remise en service intermédiaire).
...
export function getHistoriqueActif(actif, mouvements = [], prets = []) {
  ...
  // puis « Retour » ou « Perdu » selon l'issue — mirrors getHistoriqueActif()
  // (js/actifs.js:69) : le champ de liaison est getActifNumero(p), PAS p.actifId.
  const pretEvents = (prets || [])
    .filter(p => getActifNumero(p) === actif.id)
    .flatMap(p => {
      const events = [{
