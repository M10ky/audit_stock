...
export default function DemandesTable({ dept }) {
  ...
  const handleValid = (d, action) => withSubmitLock(async () => {
    // FIX Étape H bis : retour visuel par ligne (loadingRowId) pendant la
    // validation/le refus + anti-double-clic par ligne. Le finally remet
    // l'état à zéro sur tous les chemins (succès, erreur retournée, erreur
    // réseau levée, ouverture de DemAttributionModal).
                      ...
                      <td>
                        {enAttente && prodInactif ? (
                          ...
                          <div style={{ display: 'flex', gap: 4 }}>
                            <Button size="sm" icon={IconCheck}
                              loading={loadingRowId === d.id}
                              disabled={loadingRowId !== null}
                              onClick={() => handleValid(d, 'Validé')}>
                              Valider
                            </Button>
                            <Button size="icon" variant="outline"
                              loading={loadingRowId === d.id}
                              disabled={loadingRowId !== null}
                              onClick={() => handleValid(d, 'Refusé')}>
                              <IconX size={14} />
                            </Button>
                          </div>
                        ) : (
